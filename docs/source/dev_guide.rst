...
First activate the ``wps_exercise`` Conda environment and install ``pytest``.
...
.. code-block:: console
  ...
  $ make clean
  $ make install
  $ conda list -n wps_exercise --explicit > spec-file.txt
...
Make a new version of wps_exercise in the following steps:
